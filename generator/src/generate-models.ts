import {GoDefaultModelNameConstraints, GoFileGenerator} from '@asyncapi/modelina';
...
const filteredFiles: FileReadType[] = fs.readdirSync(schemaFiles).map((file) => {
  ...
  async function generateModelsForV2_0() {
    const outputDirForV2_0 = path.resolve(__dirname, outputDirPath, 'asyncapi_2_0');
    ...
    await generator.generateToFiles(input, outputDirForV2_0,{packageName: 'models'})
}
...
async function defaultGenerateModels(input: FileReadType, outputDir: string) {
    ...
    const outputDirForVersion = path.resolve(__dirname, outputDir, input.asyncapiVersion);
    ...
    await generator.generateToFiles(inputObj, outputDirForVersion, {packageName: 'models'});
  }
